...
/// The alignment of the [`dropdown_menu`].
///
/// [`Default`] is `DropdownMenuAlignement::Left`, used when no variant is given.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
#[allow(dead_code)]
pub enum DropdownMenuAlignment {
    /// The menu is aligned with the left edge of the parent element.
    #[default]
    Left,
    /// The menu is aligned with the right edge of the parent element.
    Right,
}

impl DropdownMenuAlignment {
    /// The Tailwind classes for this variant.
    fn classes(self) -> StaticClass {
        match self {
            Self::Left => class!("left-0"),
            Self::Right => class!("right-0"),
        }
    }
}
...
///         "Options"
///         icon(
///             data: iconify_icon!("mdi:chevron-down"),
...
pub async fn dropdown_menu_content(
    #[default] alignment: DropdownMenuAlignment,
    #[default] mut attrs: Attributes,
    #[default] child: Child<'_>,
) -> Result<impl View> {
    Ok(view! {
